...
namespace BuffTracker.Shared.Services;

public class StatusRulesEngine
{
    public string DisplayRemainingRounds(StatusEffect effect, int currentRound)
    {
        ...
        int remainingRounds = CalculateRemainingRounds(effect, currentRound);

        if (currentRound < effect.RoundWhenCast)
    ...
    public int CalculateRemainingRounds(StatusEffect effect, int currentRound)
    {
        ...
        if (effect.MaxDurationInRoundsOverride is not null && effect.MaxDurationInRoundsOverride > 0)
        {
            ...
            {
                remainingRounds = effect.RoundWhenCast + effect.MaxDurationInRoundsOverride.Value - currentRound;
            }
        ...
        if (effect.RoundWhenCast <= currentRound)
        {
            remainingRounds = effect.RoundWhenCast + maxRounds - currentRound;
        }
        return remainingRounds;
    }
    ...
    private int RoundsPerDurationUnit(DurationUnit unit) => unit switch
    {
        ...
        _ => throw new ArgumentException($"A duration unit of {unit} cannot be converted into rounds")
    };
}